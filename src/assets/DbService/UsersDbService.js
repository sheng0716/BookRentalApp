...
// Axios request to fetch user data by user_id
const getUserDataByUserId = async (userId) => {
    try {
        const response = await axios.get(`${pre_url}/api/users/${userId}`);
...
const updateUser = async (userId, newData) => {
    try {
        const response = await axios.put(`${pre_url}/api/users/${userId}`, newData);
